...
The physical page-turning sequence is controlled by an Arduino Uno running a low-latency state machine. The mechatronic cycle contains five sequential stages:
\begin{enumerate}
\item \textbf{Calibration Phase:} Before beginning automation, the user adjusts the travel angles for the page gripper and flipper servos using an analog potentiometer. The values are committed to the Uno's internal EEPROM, preventing recalibration between restarts.
...
\item \textbf{Capture Trigger:} At the moment of maximum hold, the Arduino Uno transmits a serial signal (\texttt{CAPTURE}) to the Raspberry Pi 5.
\end{enumerate}
...
Pi Serial TX & Pin 1 (TX) & Output & Emit CAPTURE serial trigger \\
Pi Serial RX & Pin 0 (RX) & Input & Receive automation commands \\
\hline
\end{tabular}
...
A Raspberry Pi 5 (8GB RAM) acts as the bridge, running an active serial daemon (\texttt{auto3.py}) that polls the Arduino serial port at 9600 baud. When the Arduino Uno state machine reaches the hold phase, it writes the string \texttt{CAPTURE} to the serial bus. The Pi captures this command, suspends serial polling, and executes the Android Debug Bridge (ADB) payload. The Pi is connected via USB-C to an overhead smartphone rig. Two devices, a Redmi Note 13 Pro (108MP) and a Vivo X300 Pro (50MP), serve as the cameras.
...
USB 2.0 (Port C) & Arduino Uno CP2102 & Serial communication (/dev/ttyUSB0) \\
...
Unlike the Faster R-CNN baseline in \cite{ref_paper}, which relies on a multi-stage Region Proposal Network (RPN) and RoI pooling, YOLOv8-small utilizes a single-stage, anchor-free design. Features are extracted using a CSPDarknet backbone enhanced with C2f (CSPLan with 2 convolutions) blocks. A Path Aggregation Network (PANet) neck performs multi-scale feature fusion. The detection head is decoupled, processing classification and regression as independent tasks.
