...
import { useHealth, useStatus, useUsage } from "@/hooks/use-smartscan";
            ...
            <CardContent>
              {healthLoading ? (
                ...
                <p className="text-sm text-muted-foreground text-center py-6">
                  Flask backend not responding — start it with{" "}
                ...
                <div className="divide-y divide-border/30">
                  <StatusRow
                    label={`Arduino ${health.arduino_model ?? "UNO"}`}
                    ok={health.arduino}
                    detail={
                      health.arduino_port
                        ? `Serial ${health.arduino_port} @ ${health.arduino_baud ?? "?"}`
                        : "Serial — requires physical connection"
                    }
                  />
                  <StatusRow
                  ...
                  <StatusRow
                    ...
                    ok={health.model_loaded}
                    detail={health.trocr_model_dir ?? "models/trocr/"}
                  />
                  <StatusRow
                    label="Tesseract OCR"
                    ok={health.tesseract}
                  ...
                  <StatusRow
                    ...
                    detail={
                      health.pdf_engine
                        ? `${health.pdf_engine} (required for /book/pdf)`
                        : "Required for /book/pdf"
                    }
                  />
                  <StatusRow
                    label="Gemini API"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    value: statusLoading
                      ? "—"
                      : String(status?.formulas_detected ?? 0),
                    icon: Zap,
                ...
                <div className="mt-4 pt-4 border-t border-border/30 flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                  ...
                  {usage.lastError && (
                    <span className="text-red-400">
                      Last error: {usage.lastError.slice(0, 80)}
                    </span>
                  )}
                </div>
              )}
            ...
            <CardContent>
              <div className="rounded-lg bg-zinc-950 border border-border/30 p-4 font-mono text-xs text-emerald-400 min-h-[220px] overflow-auto max-h-[380px] space-y-1">
                <p className="text-zinc-500">
                  &gt; SmartScan Processing Engine v2.0
                </p>
                <p className="text-zinc-500">
                  &gt; Flask backend — http://localhost:5000
                </p>
                <p className="text-zinc-500">
                  &gt; YOLO + TrOCR + Gemini routing active
                </p>
                ...
                {statusLoading ? (
                  ...
                  <p className="text-zinc-600">
                    &gt; No activity yet. Process images to begin.
                  </p>
                ) : (
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                {[
                  ...
                  {
                    label: "Pi IP (config.py)",
                    value: health?.pi_ip ?? "192.168.1.100",
                  },
                  {
                    label: "Models Directory",
                    value: "E:\\PROJECT\\SmartScan\\models\\",
                  },
                  {
                    label: "Captures Directory",
                    value: "E:\\PROJECT\\SmartScan\\SmartScan_Captures\\",
                  },
                  {
                    label: "Output PDF",
                    value:
                      "E:\\PROJECT\\SmartScan\\output\\pdf\\Final_Book.pdf",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="flex flex-col gap-0.5 p-2 rounded-lg bg-muted/20 border border-border/20"
                  >
                    <span className="text-muted-foreground text-[10px] uppercase tracking-wide">
                      {item.label}
                    </span>
                    <code className="text-foreground text-[11px] font-mono break-all">
                      {item.value}
                    </code>
                  </div>
                ))}
