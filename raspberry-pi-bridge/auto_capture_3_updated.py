...
# === CONFIGURATION ===
# Update PI_SAVE_PATH to your Raspberry Pi's capture directory
PI_SAVE_PATH = os.path.join(os.path.expanduser("~"), "smartscan", "captured_images")
COUNTER_FILE = os.path.join(PI_SAVE_PATH, "page_counter.txt")

# === ADB DEVICE CONFIG ===
# Run `adb devices` to find your phone serial numbers, then update below
DEVICE_PATHS = {
    "YOUR_LEFT_PHONE_SERIAL": "/storage/emulated/0/DCIM/Camera",   # LEFT phone
    "YOUR_RIGHT_PHONE_SERIAL": "/storage/emulated/0/DCIM/Camera",  # RIGHT phone
}
