...
> **Repository:** [github.com/your-username/SmartScan](https://github.com/) *(To be updated)*
...
5. [Hardware Components & Bill of Materials](#-hardware-components--bill-of-materials)
6. [Software Stack](#-software-stack)
7. [Feature Deep-Dive (With Physical Demo Plan)](#-feature-deep-dive-with-physical-demo-plan)
...
8. [Dataset Strategy (10% Subset)](#-dataset-strategy-10-subset)
9. [End-to-End Data Flow](#-end-to-end-data-flow)
10. [Physical Demo Plan](#-physical-demo-plan)
11. [Key Metrics & Expected Performance](#-key-metrics--expected-performance)
12. [Budget Analysis](#-budget-analysis)
13. [Project Timeline](#-project-timeline)
14. [Potential Faculty Questions & Prepared Answers](#-potential-faculty-questions--prepared-answers)
15. [Presentation Slide Deck (10-Min Format for 5 Speakers)](#-presentation-slide-deck-10-min-format-for-5-speakers)
...
5. **Deep Learning Objective:** Extract standard text using Tesseract while isolating and converting math formulas to LaTeX using Faster R-CNN + TrOCR *(Trained on 10% subsets of IBEM and Im2LaTeX-100K)*
...
**What it does:** A two-stage deep learning pipeline that first *detects* mathematical expressions in a page image, then *recognizes* and converts each expression into syntactically correct LaTeX code.
...
**A:** General-purpose models like Google Lens or ChatGPT Vision work on isolated, clean images. Our system handles the *entire pipeline* — from physical page turning to batch processing hundreds of pages with embedded math in mixed-content layouts. The Faster R-CNN first *locates* where math is on a dense page, then TrOCR converts only those regions. This separation of detection and recognition is what makes it scalable and accurate for full textbooks.
...
**A:** Not in the current scope. The IBEM dataset and Im2LaTeX-100K both focus on *printed* mathematical expressions. Handwritten math recognition is listed as future work in the original paper and would require retraining on datasets like MathWriting (Google) or CROHME.
...
**A:** With 10% data, we expect a ~5-8% drop in precision/recall compared to the paper's numbers. However, this is sufficient to *demonstrate the pipeline works*. For a production deployment, we would train on the full dataset. The architecture and pipeline design are identical — only the training data volume differs.
...
1. S. Thamaraiselvan, V. Venugopal, S. Vekkot, and P. K. Pisharady, "An Automated Academic Book Scanner With Deep Learning Powered Math Expression Detection and Recognition," *IEEE Access*, vol. 13, pp. 206121–206137, 2025. DOI: [10.1109/ACCESS.2025.3638780](https://doi.org/10.1109/ACCESS.2025.3638780)
2. D. Anitei et al., "The IBEM dataset," *Pattern Recognit. Lett.*, vol. 172, pp. 29–36, 2023. [Dataset: Zenodo](https://doi.org/10.5281/zenodo.4757865)
3. A. Kanervisto, "Im2latex-100k dataset," Zenodo, 2016. [Dataset](https://doi.org/10.5281/zenodo.56198)
4. M. Li et al., "TrOCR: Transformer-based optical character recognition," *Proc. AAAI*, 2023.
5. F. M. Schmitt-Koopmann et al., "MathNet: A data-centric approach for printed mathematical expression recognition," *IEEE Access*, 2024.
6. R. Smith, "An overview of the Tesseract OCR engine," *Proc. ICDAR*, 2007.
