...
def rotate_and_split(image_path, capture_num):
    ...
    # Split VERTICALLY at mid-width  ← key fix for portrait captures
    mid = w // 2
    ...
    print(f"[Split] {left_name} ({left_half.shape[1]}x{left_half.shape[0]}) | "
          f"{right_name} ({right_half.shape[1]}x{right_half.shape[0]})")
